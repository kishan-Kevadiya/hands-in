...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="sticky z-50">
      <div className="fixed top-4 left-1/2 w-[92%] md:w-[74vw] -translate-x-1/2 flex items-center justify-between bg-white/80 backdrop-blur-sm shadow-lg border border-gray-200 rounded-full px-6 py-3">
        {/* Logo */}
        <Link to={LANDING_PAGE} className="hidden lg:flex items-center">
          <img src="/logo.webp" alt="headsin" className="w-44 object-contain" />
        </Link>
                    ...
                    <>
                      {item.icon && (
                        <span className="flex items-center w-5 h-5">
                          <Star
                            fromcolor={isActive ? "#3F1562" : "#9D9D9D"}
                            tocolor={isActive ? "#DF6789" : "#9D9D9D"}
                          />
                        </span>
                      )}
            ...
            <motion.ul variants={ulVariant} className="list-none mt-10">
              {NAVBAR_ITEMS.map((navItem) => (
                ...
                >
                  ...
                  >
                    <Link to={navItem.to} onClick={() => setIsOpen(false)}>
                      {navItem.title}
                    </Link>
                  </motion.div>
                </motion.li>
