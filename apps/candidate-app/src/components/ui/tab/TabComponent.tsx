...
const TabItem: React.FC<TabItemProps> = ({
    ...
    return (
        <Link
            ...
            className={twMerge(
                "h-[50px] flex-1 flex items-center justify-center gap-2 p-2 rounded-t-xl relative cursor-pointer transition-colors duration-200",
                isActiveTab ? "bg-white" : "bg-[#F7FAFF]"
            )}
        >
            <p className="inline-flex items-center gap-2 text-base font-semibold text-black">
                <span className="z-20">{icon}</span> <span className={twMerge(`${path === "." && "bg-gradient-to-r bg-clip-text text-transparent from-[#3F1562] to-[#DF6789] "} text-sm md:text-base overflow-ellipsis whitespace-nowrap overflow-hidden`, !isActiveTab && "hidden md:inline")}>{label}</span>
            </p>
