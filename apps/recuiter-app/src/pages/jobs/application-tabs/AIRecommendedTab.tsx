...
import { getSuggestedJobs } from "@/helpers/apis/jobs";
...
const AIRecommendedTab: React.FC = () => {
    const { jobId } = useParams();

    const [dateRange, setDateRange] = useState<Nullable<(Date | null)[]>>([moment().subtract(5, "months").toDate(), moment().toDate()]);
        ...
        <div className="w-full h-full">
            <div className="w-full flex lg:flex-row flex-col items-center justify-between lg:h-[50px] mb-2">
                <div className="flex md:flex-row flex-col items-center justify-start gap-4 lg:w-auto w-11/12">
                    ...
                    >
                        <InputIcon className="m-0 -traslate-y-1/2 inline-flex"><i className="pi pi-search"></i> </InputIcon>
                        <InputText
                            value={search}
                ...
                <button
                    ...
                    onClick={() => {
                        setDateRange([moment().subtract(15, "days").toDate(), moment().add(15, "days").toDate()]);
                        setSearch("");
                    }}
                >
                    Reset
            ...
            <InfiniteScroll<SuggestedUserWithApplicationData, { page: number; pageSize: number }>
                queryKey={[
                    USE_QUERY_KEYS.GET_SUGGESTED_JOBS,
                    debouncedSearch,
                    dateRange?.[0]?.toString() as string,
                    dateRange?.[1]?.toString() as string
                ]}
                fetchFn={async (params) => {
                    const response = await getSuggestedJobs({
                        ...params,
                        jobId,
                ...
                emptyComponent={
                    <div className="w-full h-4/5 flex items-center justify-center text-primary text-lg font-semibold text-center">No AI Recommended Applications Found.</div>
                }
                gridCols={2}
                direction="down"
...
export default AIRecommendedTab;
