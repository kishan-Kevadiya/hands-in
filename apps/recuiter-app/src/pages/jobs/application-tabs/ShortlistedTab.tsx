...
import { ApplicationStatus, SuggestedUserWithApplicationData } from "@/types/applications.types";
...
const ShortlistedTab: React.FC = () => {
    const { jobId } = useParams();

    const [dateRange, setDateRange] = useState<Nullable<(Date | null)[]>>([moment().subtract(15, "days").toDate(), moment().add(15, "days").toDate()]);
        ...
        <div className="w-full h-full">
            ...
            <InfiniteScroll<SuggestedUserWithApplicationData, { page: number; pageSize: number }>
                ...
                renderItem={(application) => (
                    <ApplicationCard key={application?.id} data={application} btnType="SHORTLISTED" />

                )}
                loadingComponent={<Loader isVisible={true} />}
                emptyComponent={
                    <div className="w-full h-4/5 flex items-center justify-center text-primary text-lg font-semibold text-center">No Shortlisted Applications Found.</div>
                }
                gridCols={2}
                direction="down"
